...
pub fn seed_fixtures(db: &storage::Database) -> storage::Result<()> {
    ...
    for f in &lib.feeds {
        let url = f.website.clone().unwrap_or_else(|| format!("https://example.org/{}.xml", f.id));
        let fid = db.add_feed("local", &url, &f.title, f.website.as_deref(), &f.accent)?;
        let gids: Vec<i64> = f.groups.iter().filter_map(|g| group_ids.get(g).copied()).collect();
    ...
    for a in &lib.articles {
        by_feed.entry(a.feed_id.clone()).or_default().push(storage::NewArticle {
            id: a.id.clone(),
            title: a.title.clone(),
    ...
    for (feed_key, items) in by_feed {
        let Some(fid) = feed_ids.get(&feed_key).copied() else { continue };
        db.upsert_articles(fid, &items, now)?;
    }
    for a in &lib.articles {
        let Some(fid) = feed_ids.get(&a.feed_id).copied() else { continue };
        if !a.unread || a.saved {
